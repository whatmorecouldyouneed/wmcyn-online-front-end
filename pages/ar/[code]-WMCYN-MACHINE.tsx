...
import { productSetToMarkerConfig, validateBackendProductSet } from '@/utils/markerSync';
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            if (productSetResponse.ok) {
              const productSetData: ProductSet = await productSetResponse.json();
              ...
              // validate backend product set structure and mindar compatibility
              const validation = validateBackendProductSet(productSetData);
              if (!validation.isValid) {
                console.error('[ARByCode] backend validation errors:', validation.errors);
              }
              if (validation.warnings.length > 0) {
                console.warn('[ARByCode] backend validation warnings:', validation.warnings);
              }
              
              // convert product set to marker config using utility function
              // this handles mindar compatibility validation per public/patterns/README.md
              const markerConfig = productSetToMarkerConfig(productSetData);
              
              if (!markerConfig) {
                // product set doesn't have ar configuration, use fallback
                console.warn('[ARByCode] Product set has no ar configuration, using fallback');
                setConfig({
                  markerType: 'custom',
                  markerDataUrl: '/patterns/pattern-wmcyn_logo_full.patt',
                  overlays: [DEFAULT_OVERLAY],
                  meta: {
                    title: productSetData.name || 'WMCYN AR Experience',
                    description: productSetData.description,
                    actions: []
                  }
                });
              } else {
                // convert marker config to resolved ar config format
                const markerType = markerConfig.markerType === 'nft' ? 'mind' : 'custom';
                const markerDataUrl = markerConfig.mindTargetSrc || markerConfig.patternUrl || '/patterns/pattern-wmcyn_logo_full.patt';
                
                // extract metadata
                const metadata = markerConfig.metadata as any;
                const metaConfig = {
                  title: metadata?.title || 'WMCYN AR Experience',
                  description: metadata?.description,
                  actions: metadata?.actions || [],
                  createdAt: metadata?.createdAt || productSetData.createdAt,
                  campaign: metadata?.campaign || productSetData.campaign
                };
                
                console.log('[ARByCode] Built metaConfig from markerConfig:', metaConfig);
