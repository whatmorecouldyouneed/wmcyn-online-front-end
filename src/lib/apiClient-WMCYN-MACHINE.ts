...
async function adminApiFetch<T = any>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      ...
      // provide more specific error for 404s
      if (path.includes('/ar-sessions')) {
        throw new Error(`AR Sessions endpoint not found (404). The backend endpoint ${path} may not be deployed. Please check:\n1. Backend Cloud Function is deployed\n2. Endpoint path is correct\n3. Backend supports AR Sessions API`);
      }
      throw new Error(`Resource not found (404): ${path}`);
    } else if (res.status === 403) {
      throw new Error('Forbidden');
    } else if (res.status === 503) {
      throw new Error('Backend service unavailable - the Cloud Function may be cold-starting or not deployed. Please try again in a few moments.');
...
// per backend docs: GET /v1/productSets?campaign=xxx&q=search&limit=50&offset=0
export const getProductSets = async (filters?: { campaign?: string; q?: string; limit?: number; offset?: number }): Promise<ProductSetsResponse> => {
  ...
  const response = await adminApiFetch(`/v1/productSets${queryString ? `?${queryString}` : ''}`);
...
export const createProductSet = async (data: CreateProductSetRequest): Promise<ProductSet> => {
  // transform items to use 'qty' field per backend spec
  const cleanItems = data.items.map(item => {
    const cleanItem: any = {
      productId: item.productId || 'default-product',
      qty: item.quantity || item.qty || 1 // backend uses 'qty'
    };
    if (item.variantId) cleanItem.variantId = item.variantId;
    if (item.maxPerUser) cleanItem.maxPerUser = item.maxPerUser;
    if (item.metadata) cleanItem.metadata = item.metadata;
    return cleanItem;
  });
  
  // build request body per backend spec
  const transformedData: any = {
    name: data.name, // required
    items: cleanItems // required, must have at least 1 item
  };
  
  // add optional fields per backend spec
  if (data.campaign) transformedData.campaign = data.campaign;
  if (data.tags && data.tags.length > 0) transformedData.tags = data.tags;
  if (data.checkoutMode) {
    transformedData.checkoutMode = data.checkoutMode; // backend uses checkoutMode
    if (data.checkoutMode === 'DISCOUNT_CODE' && data.discountCode) {
      transformedData.discountCode = data.discountCode;
    }
  }
  // legacy checkout field support (for backward compatibility)
  if (data.checkout && !data.checkoutMode) {
    // map legacy checkout to checkoutMode
    if (data.checkout.cartLink) {
      transformedData.checkoutMode = 'SHOPIFY_CART_LINK';
    } else if (data.checkout.discountCode) {
      transformedData.checkoutMode = 'DISCOUNT_CODE';
      transformedData.discountCode = data.checkout.discountCode;
    } else {
      transformedData.checkoutMode = 'NONE';
    }
  }
  
  console.log('[createProductSet] Sending data per backend spec:', JSON.stringify(transformedData, null, 2));
...
// per backend docs: PATCH /v1/productSets/:id
...
export const getProductSetStats = async (id: string): Promise<ProductSetStatsResponse> => {
  return apiFetch(`/v1/productSets/${id}/stats`);
};
...
// per backend docs: POST /v1/qrcodes/generate
...
// per backend docs: GET /v1/qrcodes?productSetId=xxx (admin)
...
// per backend docs: GET /v1/qrcodes/:code (public endpoint)
export const getQRCodeData = async (code: string): Promise<QRCodeData> => {
  return apiFetch(`/v1/qrcodes/${encodeURIComponent(code)}`);
};

// per backend docs: DELETE /v1/qrcodes/:code (admin)
export const deleteQRCode = (code: string): Promise<void> => 
  adminApiFetch(`/v1/qrcodes/${code}`, {
    method: 'DELETE'
  });
...
export const arSessions = {
  // get ar session data for viewer (public endpoint)
  // per backend docs: GET /api/ar-sessions/:id/data
  get: async (id: string): Promise<ARSessionData> => {
    return apiFetch(`/api/ar-sessions/${id}/data`);
  },

  // get ar session (admin endpoint)
  // per backend docs: GET /v1/ar-sessions/:id
  getAdmin: async (id: string): Promise<ARSessionData> => {
    return adminApiFetch(`/v1/ar-sessions/${id}`);
  },

  // list ar sessions (admin)
  // per backend docs: GET /v1/ar-sessions?status=active&campaign=xxx&productId=xxx&limit=50
  list: async (filters?: { status?: string; campaign?: string; productId?: string; limit?: number }): Promise<ARSessionListResponse> => {
    const params = new URLSearchParams();
    ...
    if (filters?.limit) params.append('limit', filters.limit.toString());
    const queryString = params.toString();
    const response = await adminApiFetch(`/v1/ar-sessions${queryString ? `?${queryString}` : ''}`);
...
export const markerPatterns = {
  ...
  // per backend docs: GET /v1/marker-patterns?type=upload&status=active&createdBy=xxx&limit=50
  list: async (filters?: { type?: 'upload' | 'template' | 'generate'; status?: 'active' | 'processing' | 'failed'; createdBy?: string; limit?: number }): Promise<MarkerPatternListResponse> => {
    const params = new URLSearchParams();
    ...
    if (filters?.limit) params.append('limit', filters.limit.toString());
    const queryString = params.toString();
    return adminApiFetch(`/v1/marker-patterns${queryString ? `?${queryString}` : ''}`);
  },

  // get marker pattern (admin)
  // per backend docs: GET /v1/marker-patterns/:id
  get: async (id: string): Promise<MarkerPattern> => {
    return adminApiFetch(`/v1/marker-patterns/${id}`);
  },

  // upload marker pattern (admin)
  // per backend docs: POST /v1/marker-patterns/upload
  ...
  // per backend docs: PUT /v1/marker-patterns/:id
  update: async (id: string, updates: { name?: string; status?: 'active' | 'inactive' }): Promise<MarkerPattern> => {
    return adminApiFetch(`/v1/marker-patterns/${id}`, {
      method: 'PUT',
      body: JSON.stringify(updates)
    });
  ...
  // get .mind file (public endpoint)
  // per backend docs: GET /api/marker-patterns/:id.mind
  getMindFile: async (id: string): Promise<string> => {
    const response = await fetch(`${API_BASE}/api/marker-patterns/${id}.mind`, {
      method: 'GET',
      credentials: 'omit'
    });
    if (!response.ok) throw new Error(`Failed to get .mind file: ${response.status}`);
    // returns redirect to Firebase Storage URL
    return response.url;
  },

  // get preview image (public endpoint)
  // per backend docs: GET /api/marker-patterns/:id.png
  getPreviewImage: async (id: string): Promise<string> => {
    const response = await fetch(`${API_BASE}/api/marker-patterns/${id}.png`, {
      method: 'GET',
      credentials: 'omit'
    });
    if (!response.ok) throw new Error(`Failed to get preview image: ${response.status}`);
    // returns redirect to Firebase Storage URL
    return response.url;
  },
