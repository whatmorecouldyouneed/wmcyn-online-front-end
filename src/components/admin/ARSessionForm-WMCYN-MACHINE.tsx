...
export default function ARSessionForm({
  ...
  const validateForm = (): boolean => {
    ...
    // productId is required per backend API spec
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // format request per backend API spec: POST /v1/ar-sessions/create
    const submitData: CreateARSessionRequest = {
      productId: formData.productId.trim(), // required per backend
      markerPattern: {
        patternId: formData.selectedPatternId,
        type: 'mind' // backend uses 'mind' type for MindAR .mind files
      },
      metadata: {
        title: formData.metadata.title.trim(),
        description: formData.metadata.description.trim(),
        actions: formData.metadata.actions.map(action => ({
    ...
    try {
      ...
      const errorMessage = error?.message || 'failed to create ar session. please try again.';
      setSubmitError(errorMessage);
      
      // if it's a 404 error, provide more helpful message
      if (errorMessage.includes('404') || errorMessage.includes('not found')) {
        setSubmitError(
          'backend endpoint not found. the ar sessions api may not be deployed. ' +
          'please check that the backend cloud function is deployed and the endpoint exists.'
        );
      }
    }
  };
